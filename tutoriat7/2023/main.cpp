...
public:
    ...
    void afiseazaDrumurile() const {
        for (const auto& drum : drumuri) {
            std::cout << drum->lungimeTronson() << "\n";
        }
    }
    void afisareMeniu() {
        int optiune;
        std::cout << "------Meniu-------\n";
        std::cout << "1. Citeste drum\n";
        std::cout << "2. Citeste contract\n";
        std::cout << "3. Afiseaza drumurile\n";
        std::cout << "4. Afiseaza contractele\n";

        while (true) {
            try {
                std::cin >> optiune;
                if (optiune < 1 || optiune > 4) throw std::invalid_argument("optiune invalida");
                break;
            }
